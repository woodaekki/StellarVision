  ...
  <div class="video-edit-page">
    ...
    <div class="video-info-section" v-if="video">
      <div class="video-preview">
        <img :src="video.thumbnail" alt="비디오 썸네일" class="video-thumbnail"/>
      </div>
    ...
    <div class="tags-section">
      ...
      <div class="add-tag-section">
        ...
        <p class="tag-help">태그는 최대 20자까지 입력 가능합니다.</p>
      </div>
    </div>
...
const fetchVideoData = async () => {
  try {
    // 비디오 정보 가져오기 (가상의 API - 실제 API 엔드포인트에 맞게 수정하세요)
    const videoResponse = await commonApi.get(`/videos/${videoId}`);
...
const fetchTags = async () => {
  try {
    const response = await commonApi.get(`/videos/${videoId}/tags`);
...
const addTag = async () => {
  ...
  // 중복 체크
  if (tags.value.some(tag => tag.tagName === tagName)) {
    alert('이미 존재하는 태그입니다.');
    return;
  }
  ...
  try {
    const response = await commonApi.post(`/videos/${videoId}/tags`, {
      tagName: tagName
    });
...
const removeTag = async (tagId) => {
  ...
  try {
    // 태그 삭제 API 호출 (실제 API 엔드포인트에 맞게 수정하세요)
    const response = await commonApi.delete(`/videos/${videoId}/tags/${tagId}`);
